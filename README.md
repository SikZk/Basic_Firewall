...
- **host1 (client1)**
  - `net10_1`: `172.20.0.2/24`
  - Default gateway: `172.20.0.1` (firewall)
- **host2 (client2)**
  - `net10_2`: `172.21.0.2/24`
  - Default gateway: `172.21.0.1` (firewall)
- **firewall (host3)**
  - `net10_1`: `172.20.0.1/24`
  - `net10_2`: `172.21.0.1/24`
  - `lan`: `172.22.1.70/24` via macvlan on `wlp131s0`
  - Default route: `172.22.1.2`
...
export LAN_SUBNET=172.22.1.0/24
export LAN_GW=172.22.1.2
export FIREWALL_LAN_IP=172.22.1.70
docker compose up --build
```
...
Some systems cannot reach macvlan containers directly from the host. If your host (`172.22.1.20`) cannot ping the firewall (`172.22.1.70`), create a host-side macvlan interface:

```bash
sudo ip link add macvlan0 link wlp131s0 type macvlan mode bridge
sudo ip addr add 172.22.1.71/24 dev macvlan0
sudo ip link set macvlan0 up
sudo ip route add 172.22.1.70/32 dev macvlan0
```
...
- A simple HTTP server (`python3 -m http.server 80`) is started so `curl` checks can hit `172.20.0.1` and `172.21.0.1`.
...
docker compose exec host1 ping -c 3 172.20.0.1
docker compose exec host1 curl -s http://172.20.0.1/

# host2 -> firewall (ping + curl)
docker compose exec host2 ping -c 3 172.21.0.1
docker compose exec host2 curl -s http://172.21.0.1/

# host1 -> host2 should be blocked by default
docker compose exec host1 ping -c 3 172.21.0.2
...
# (host side)
ping -c 3 172.22.1.70
# (firewall side)
docker compose exec firewall ping -c 3 172.22.1.20
```

## Notes

- `net10_1` and `net10_2` are isolated internal networks; there is no direct bridge to your LAN.
- Inter-subnet traffic (`172.20.0.0/24` ↔ `172.21.0.0/24`) is blocked by default. Update `resources/config.json` and/or the firewall rules if you want to allow it.
